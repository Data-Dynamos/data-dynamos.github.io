...
In this exercise, you will use dbt to perform an INNER JOIN between the `aggregate_global_emissions` and `aggregate_global_temperatures` views to analyze the relationship between global emissions and temperatures. The result of the join will be stored in a table called `aggregate_global_emissions_temperatures` in the `global_temperatures` schema.
...
6. In the `select` statement, perform an INNER JOIN between the `aggregate_global_emissions` and `aggregate_global_temperatures` views on the "Year" column.
